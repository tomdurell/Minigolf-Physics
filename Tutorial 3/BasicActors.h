...
namespace PhysicsEngine
{
	...
	{
	public:
		Platform(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(1.0f, 1.0f, 1.0f), PxReal density = 1.f) : StaticActor(pose)
		{
			CreateShape(PxBoxGeometry(dimensions.x, (dimensions.y / 10.f), dimensions.z), density);
			CreateShape(PxBoxGeometry((dimensions.x / 10.f), (dimensions.y / 5.f), dimensions.z), density);
			CreateShape(PxBoxGeometry((dimensions.x / 10.f), dimensions.y / 5.f, dimensions.z), density);
			GetShape(0)->setLocalPose(PxTransform(PxVec3(.0f, .0f, .0f)));
			GetShape(1)->setLocalPose(PxTransform(PxVec3(10.0f, .0f, 0.0f)));
			GetShape(2)->setLocalPose(PxTransform(PxVec3(-10.f, .0f, 0.0f)));

		}
	};

	class startPlatform : public StaticActor
	{
	public:
		startPlatform(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(1.0f, 1.0f, 1.0f), PxReal density = 1.f) : StaticActor(pose)
		{
			CreateShape(PxBoxGeometry(dimensions.x, (dimensions.y / 10.f), dimensions.z), density);
			CreateShape(PxBoxGeometry((dimensions.x / 10.f), (dimensions.y / 3.f), dimensions.z), density);
			CreateShape(PxBoxGeometry((dimensions.x / 10.f), dimensions.y / 3.f, dimensions.z), density);
			CreateShape(PxBoxGeometry(dimensions.x, dimensions.y / 5.f, (dimensions.z / 35.f)), density);
			GetShape(0)->setLocalPose(PxTransform(PxVec3(.0f, .0f, .0f)));
			//GetShape(0)->setMaterial();
			GetShape(1)->setLocalPose(PxTransform(PxVec3(10.0f, .0f, 0.0f)));
			GetShape(2)->setLocalPose(PxTransform(PxVec3(-10.f, .0f, 0.0f)));
			GetShape(3)->setLocalPose(PxTransform(PxVec3(0.f, 0.f, -49.f)));

		}
	};
	class Hole : public StaticActor
	{
	public:
		Hole(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(1.0f, 1.0f, 1.0f), PxReal density = 1.f) : StaticActor(pose)
		{
			CreateShape(PxBoxGeometry(dimensions.x/ 2.f, (dimensions.y / 5.f), dimensions.z), density);
			CreateShape(PxBoxGeometry((dimensions.x/ 2.f), (dimensions.y / 5.f), dimensions.z), density);
			CreateShape(PxBoxGeometry((dimensions.x), dimensions.y / 5.f, dimensions.z/2.f), density);
			CreateShape(PxBoxGeometry(dimensions.x, dimensions.y / 5.f, (dimensions.z /2.f)), density);
			GetShape(0)->setLocalPose(PxTransform(PxVec3(0.f, .0f, .0f)));
			GetShape(1)->setLocalPose(PxTransform(PxVec3(-8.0f, .0f, 0.0f)));
			GetShape(2)->setLocalPose(PxTransform(PxVec3(-4.f, 0.0f, 4.f)));
			GetShape(3)->setLocalPose(PxTransform(PxVec3(-4.f, 0.f, -4.f)));

		}
	};
	class cornerPlatform : public StaticActor
	{
	public:
		cornerPlatform(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(1.0f, 1.0f, 1.0f), PxReal density = 1.f) : StaticActor(pose)
		{
			CreateShape(PxBoxGeometry(dimensions.x, (dimensions.y / 10.f), dimensions.z), density);
			CreateShape(PxBoxGeometry((dimensions.x / 10.f), (dimensions.y / 5.f), dimensions.z), density);
			//CreateShape(PxBoxGeometry((dimensions.x / 10.f), dimensions.y / 5.f, dimensions.z), density);
			...
			GetShape(1)->setLocalPose(PxTransform(PxVec3(10.0f, .0f, 0.0f)));
			//GetShape(2)->setLocalPose(PxTransform(PxVec3(-10.f, .0f, 0.0f)));
			GetShape(2)->setLocalPose(PxTransform(PxVec3(0.f, 0.f, -49.f)));

		}
	};
	class pendulum : public StaticActor
	{
	public:
		pendulum(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(1.0f, 1.0f, 1.0f), PxReal density = 1.f) : StaticActor(pose)
		{
			CreateShape(PxBoxGeometry(dimensions.x / 10.f, (dimensions.y), dimensions.z / 10.f), density);
			CreateShape(PxBoxGeometry((dimensions.x / 5.f), (dimensions.y / 8.f), dimensions.z / 5.f), density);
			GetShape(0)->setLocalPose(PxTransform(PxVec3(.0f, .0f, .0f)));
			GetShape(1)->setLocalPose(PxTransform(PxVec3(.0f, -10.0f, 0.0f)));
			//GetShape(0)->setMaterial();
	...
	{
	public:
		Ramp(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(1.0f, 1.0f, 1.0f), PxReal density = 1.f) : StaticActor(pose)
		{
			CreateShape(PxBoxGeometry((dimensions.x / 2.5f), (dimensions.y / 10.f), dimensions.z), density);
			GetShape(0)->setLocalPose(PxTransform(PxVec3(0.f, .0f, 0.0f)));
		}
	};
	...
	class Slider : public DynamicActor
	{
	public:
		//a Box with default parameters:
		// - pose in 0,0,0
		// - dimensions: 1m x 1m x 1m
		// - denisty: 1kg/m^3
		Slider(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(.5f, .5f, .5f), PxReal density = 1.f)
			: DynamicActor(pose)
		{
			CreateShape(PxBoxGeometry(dimensions), density);
		}
	...
	public:
		...
		Bumper(const PxTransform& pose = PxTransform(PxIdentity), PxReal radius = 1.f, PxReal density = 1.f)
			: StaticActor(pose)
		{
	...
	public:
		...
		Cloth(PxTransform pose = PxTransform(PxIdentity), const PxVec2& size = PxVec2(1.f, 1.f), PxU32 width = 1, PxU32 height = 1, bool fix_top = true)
		{
			...
			PxClothParticle* vertices = new PxClothParticle[(width + 1)*(height + 1) * 900];
