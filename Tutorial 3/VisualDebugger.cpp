...
namespace VisualDebugger
{
	...
	PxRigidActor* ballLocation;
	PxRigidDynamic* ballStopper;
	int shotCount = 0;
	...
	void Init(const char *window_name, int width, int height)
	{
		...
		camera = new Camera(PxVec3(0.0f, 5.0f, 15.0f), PxVec3(0.f, -.1f, -1.f), 20.f);
	...
	void HUDInit()
	{
		...
		hud.AddLine(HELP, "Shotpower: " + std::to_string(shotPower/5.f));
		hud.AddLine(HELP, "Par: 6 ");
		hud.AddLine(HELP, "Strokes: " + std::to_string(shotCount));
		hud.AddLine(HELP, "Shoot = P");
		hud.AddLine(HELP, "Teleport to Ball = T");
		if (scene->my_callback->winCondition == true)
		{
			hud.AddLine(HELP, "Congratulations! You beat the course in: " + std::to_string(shotCount)+ " shots!") ;
			if (shotCount > 6)
			{
				hud.AddLine(HELP, "Thats " + std::to_string(shotCount - 6) + "shots over par! Better luck next time!");
			}
			if (shotCount < 6)
			{
				hud.AddLine(HELP, "Thats " + std::to_string(6 - shotCount) + " shots under par! You must be Tiger, of House Woods, First of His Name, the Unput, King of the Golf Course and the Rich Men, Khal of the Great Grass Sea, Breaker of Clubs, and Father of the putting green. !");
			}
			if (shotCount == 6)
			{
				hud.AddLine(HELP, "Thats bang on par, not bad, but git gudder");
			}
		}
	...
	void RenderScene()
	{
		...
		ballStopper = (PxRigidDynamic*)scene->golfBall->Get();
		ballLocation = (PxRigidActor*)scene->golfBall->Get();
		ballLocation->getGlobalPose();
	...
	void UserKeyPress(int key)
	{
		...
		case 'P': //hit ball
			if (ballStopper->isSleeping() == true)
			{
				scene->golfBall->Get()->isRigidBody()->addForce(camera->getDir() * (shotPower * 100));
				//scene->golfBall->Get()->isRigidBody()->addForce(PxVec3(1.0f,0.f,0.0f) * (shotPower * 100));
				shotCount++;
				HUDInit();
			}
			else
			...
			break;
		case 'T': //teleport camera
		{
			camera->eye = ballLocation->getGlobalPose().p + PxVec3(0.f, 17.f, -30.f);
		}
		default:
			break;
	...
	void ForceInput(int key)
	{
		...
		case '{': //hit ball
			scene->golfBall->Get()->isRigidBody()->addForce(camera->getDir() * 50);
			break;
		default:
			break;
		}
